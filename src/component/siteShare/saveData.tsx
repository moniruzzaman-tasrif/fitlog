      ...
      {saveProps.map(item => (
        <div className="w-full" key={item.id}>
          <div className="card bg-[#14171E] p-4 border border-gray-800 rounded-3xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4 w-full">
            ...
            <div className="flex items-center justify-between md:justify-center gap-2 w-full md:w-auto pt-2 md:pt-0 border-t md:border-t-0 border-gray-800">
              <Link
                href={`/WorkOuts/${item.id}`}
                className="flex-1 md:flex-initial"
              >
                <button className="btn border border-gray-800/80 shadow-none bg-[#14171E] hover:bg-[#1f242d] text-gray-300 text-xs rounded-2xl w-full md:w-auto px-4">
                  View Details
                </button>
              </Link>
